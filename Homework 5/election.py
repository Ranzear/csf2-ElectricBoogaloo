...
def state_edges(election_result_rows):
    ...
    for row in election_result_rows:
        output[row["State"]] = row_to_edge(row) #Row row row your boat.
    return output
...
def most_recent_poll_row(poll_rows, pollster, state):
    ...
    for row in poll_rows:
        if row["Pollster"] == pollster and row["State"] == state:
            if recent == None:
                recent = row
            elif earlier_date(recent["Date"], row["Date"]):
                recent = row
    return recent
...
def pollster_predictions(poll_rows):
    ...
    for pollster in pollsters:
        ...
        for state in states:
            valid_row = None
            valid_row = most_recent_poll_row(poll_rows, pollster, state)
            if valid_row:
                edges = state_edges([valid_row])
                prediction[pollster][state] = edges[state]
    return prediction

    # and here is where I broke **EVERYTHING** last time!
